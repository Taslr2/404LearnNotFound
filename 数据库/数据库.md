...
e.g.**create domain** DDollars **as numeric(12,2) not null；**
...
**grant references**(depat_name) **on** department **to** Mariano
...
除了按item_name,color分组输出结果，还会输出item_name,color为null时的元组
...
- **派生属性**：这类属性的值可以从其他相关属性或实体的值派生出来，e.g.instructor实体集有date_of_birth属性，由此可以算出age属性
